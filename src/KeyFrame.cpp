...
KeyFrame::KeyFrame(Frame &F, Map *pMap):
    ...
    mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn), imgTnsr(F.imgTnsr), keypoints(F.keypoints), scores(F.scores), descriptors(F.descriptors),
